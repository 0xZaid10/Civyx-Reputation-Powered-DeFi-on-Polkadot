      ...
      {done ? (
        ...
        <>
          ...
          <button
            ...
            disabled={isPending || isConfirming || !amount || (mode === 'withdraw' && maxWithdraw === 0n)}
          ...
          {isConfirming && txHash && (
            <div className="mt-2 text-xs text-gray-500">
              ...
              <button onClick={() => { setDone(true); setLastTx(txHash); refetch(); }}
...
function TaskHistory({ commitment }: { commitment: `0x${string}` }) {
  ...
  const { data: tasksCompleted } = useReadContract({
    address:      CONTRACTS.TaskRewardDispenser as `0x${string}`,
    abi:          DISPENSER_ABI,
    functionName: 'tasksCompletedBy',
    args:         [commitment],
    query:        { enabled: commitment !== ('0x' + '0'.repeat(64)) },
  });

  const claimed = results.data?.map(r => r.result as boolean) ?? [];
  const completedCount = claimed.filter(Boolean).length;
  const totalPts = TASK_DEFS.reduce((sum, t, i) => sum + (claimed[i] ? t.pts : 0), 0);

  return (
    <div className="border border-gray-200 rounded-lg p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm font-semibold text-gray-900">Completed Tasks</div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400">{completedCount}/{TASK_DEFS.length} tasks</span>
          {totalPts > 0 && (
      ...
      <div className="space-y-2">
        {TASK_DEFS.map((task, i) => {
          const done = claimed[i] ?? false;
          return (
            <div key={task.id} className={`flex items-center justify-between rounded-lg px-3 py-2.5 border ${
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-xs font-bold ${done ? 'text-green-600' : 'text-gray-300'}`}>
                  +{task.pts} pts
                </span>
                {done && (CONTRACTS as any)[task.contract] && (
                  <a
                    href={blockscoutAddress((CONTRACTS as any)[task.contract])}
                    target="_blank"
                    rel="noopener noreferrer"
      ...
      {completedCount === 0 && (
        <p className="text-xs text-gray-400 text-center mt-3">
          No tasks completed yet. Visit the <a href="/community" className="text-green-600 hover:underline">Community page</a> to get started.
        </p>
      )}
    </div>
  );
    ...
    <div className="min-h-screen bg-white px-4 py-12">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className={`border rounded-lg p-4 ${identity.hasSecret ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
          <div className="flex items-center gap-2">
            <span className={`text-sm ${identity.hasSecret ? 'text-green-700' : 'text-amber-700'}`}>
              {identity.hasSecret ? '✓ Secret found in browser storage' : '⚠ Secret not found in this browser'}
            </span>
          </div>
          {!identity.hasSecret && (
            <p className="text-xs text-amber-600 mt-1">
              Import your secret file to link new wallets or prove ownership.
            </p>
          )}
        </div>
        ...
        {/* Task History */}
        <TaskHistory commitment={identity.commitment} />
